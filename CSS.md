...
<<<<<<< HEAD
24. scrollTop获得当前屏幕中左上角距离容器顶部的距离，position获得元素相对最近已定位元素的位置偏移量，offset获得元素相对与整个文档的位置偏移量。

25. 判断屏幕是否滚动到底部，可以通过判断 当前滚动位置+\$(window).height()-其他容器外fixed元素高度==容器高度 是否成立。
=======
24. 使用find指定'option:checked'可以得到被选中的select选项。

25. 

>>>>>>> 46d61282d3aef977de4df8816f67c838a5a78467
